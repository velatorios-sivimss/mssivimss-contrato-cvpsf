...
public class ReportePagoAnticipado {
	...
	public String generaReporte(ReporteDto reporteDto) {
		...
		.append("  , PQ.REF_PAQUETE_NOMBRE AS nomPaquete ").append("  ,(SELECT GROUP_CONCAT(CONCAT('$', IFNULL(FORMAT(PAGO.IMP_MONTO_MENSUAL,2) ,''),' - ',IFNULL(DATE_FORMAT(PAGO.FEC_PARCIALIDAD, '%d-%m-%Y'),'') ) SEPARATOR '\\n') FROM SVT_PAGO_SFPA PAGO JOIN SVT_PLAN_SFPA PLAN ON PLAN.ID_PLAN_SFPA = PAGO.ID_PLAN_SFPA WHERE PAGO.ID_PLAN_SFPA  =  " + reporteDto.getIdPlanSFPA() + " ) AS pagos ")
		...
		.append("  , P5.REF_CORREO AS correoVelatorio ").append("  , CTE.ID_CONTRATANTE AS numeroAfiliacion ").append("  , CONCAT(IFNULL(P2.NOM_PERSONA,''),' ',IFNULL(P2.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P2.NOM_SEGUNDO_APELLIDO,'')) AS nombreSustituto ").append(" , P2.FEC_NAC AS fecNacSustituto")
		.append("  , P2.CVE_RFC AS rfcSustituto ").append("  , P2.REF_TELEFONO AS telefonoSustituto  ").append("  , CONCAT(IFNULL(DO2.REF_CALLE,''),' ',IFNULL(DO2.NUM_EXTERIOR,''), ' ',IFNULL(DO2.NUM_INTERIOR,''),' ',IFNULL(DO2.REF_CP,''), ' ',IFNULL(DO2.REF_COLONIA,''),' ',IFNULL(DO2.REF_MUNICIPIO,''), ' ',IFNULL(DO2.REF_ESTADO,'')) AS direccionSustituto ")
		.append("  , CONCAT(IFNULL(P3.NOM_PERSONA,''),' ',IFNULL(P3.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P3.NOM_SEGUNDO_APELLIDO,'')) AS nombreB1 ").append(" , DATE_FORMAT(P3.FEC_NAC, '%d-%m-%Y') AS fecNacB1 ").append("  , P3.CVE_RFC AS rfcB1 ").append("  , P3.REF_TELEFONO AS telefonoB1 ")
		.append("  , CONCAT(IFNULL(DO3.REF_CALLE,''),' ',IFNULL(DO3.NUM_EXTERIOR,''), ' ',IFNULL(DO3.NUM_INTERIOR,''),' ',IFNULL(DO3.REF_CP,''), ' ',IFNULL(DO3.REF_COLONIA,''),' ',IFNULL(DO3.REF_MUNICIPIO,''), ' ',IFNULL(DO3.REF_ESTADO,'')) AS direccionB1")
		.append("  , CONCAT(IFNULL(P4.NOM_PERSONA,''),' ',IFNULL(P4.NOM_PRIMER_APELLIDO,''), ' ',IFNULL(P4.NOM_SEGUNDO_APELLIDO,'')) AS nombreB2 ").append("  , DATE_FORMAT(P4.FEC_NAC, '%d-%m-%Y') AS fecNacB2 ").append("  , P4.CVE_RFC AS rfcB2 ").append("  , P4.REF_TELEFONO AS telefonoB2 ")
		.append("  , CONCAT(IFNULL(DO4.REF_CALLE,''),' ',IFNULL(DO4.NUM_EXTERIOR,''), ' ',IFNULL(DO4.NUM_INTERIOR,''), ' ',IFNULL(DO4.REF_CP,''),' ',IFNULL(DO4.REF_COLONIA,''),' ',IFNULL(DO4.REF_MUNICIPIO,''), ' ',IFNULL(DO4.REF_ESTADO,'')) AS direccionB2 ")
		...
		.append(" JOIN SVC_VELATORIO VO ON VO.ID_VELATORIO = PSFPA.ID_VELATORIO ").append(" JOIN SVT_USUARIOS US ON US.ID_USUARIO = VO.ID_USUARIO_ADMIN ").append(" LEFT JOIN  SVC_PERSONA P5 ON P5.ID_PERSONA  = US.ID_PERSONA ")
		.append(" JOIN SVT_DOMICILIO DO ON DO.ID_DOMICILIO = CTE.ID_DOMICILIO ").append(" LEFT JOIN SVT_TITULAR_BENEFICIARIOS TBSP1 ON TBSP1.ID_TITULAR_BENEFICIARIOS  = PSFPA.ID_TITULAR_SUBSTITUTO ")
