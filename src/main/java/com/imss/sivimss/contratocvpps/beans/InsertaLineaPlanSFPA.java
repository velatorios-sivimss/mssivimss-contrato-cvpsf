...
@Slf4j
public class InsertaLineaPlanSFPA implements Serializable {
	...
	public InsertPlanSfpaRequest insertaLineaPlanSfpa(PlanSFPARequest planSFPARequest,UsuarioDto usuarioDto) {
		...
		for (ContratanteRequest contratanteRequest : planSFPARequest.getTitularesBeneficiarios()) {
			log.info("  idPersona:  " + contratanteRequest.getIdPersona() + "  idContratante:  "
					+ contratanteRequest.getIdTitularBeneficiarios() + "  IdDomicilio:  "
					+ contratanteRequest.getCp().getIdDomicilio());
	...
	private String insertPersona(ContratanteRequest contratanteRequest, UsuarioDto usuarioDto) {
		...
		q.agregarParametroValues(ConsultaConstantes.NOM_PRIMER_APELLIDO,
				SelectQueryUtil.setValor(contratanteRequest.getPrimerApellido()));
		q.agregarParametroValues(ConsultaConstantes.NOM_SEGUNDO_APELLIDO,
				SelectQueryUtil.setValor(contratanteRequest.getSegundoApellido()));
	...
	public String insertDomicilio(ContratanteRequest contratanteRequest, UsuarioDto usuarioDto) {
		...
		q.agregarParametroValues("REF_MUNICIPIO",
				SelectQueryUtil.setValor(contratanteRequest.getCp().getDesMunicipio()));
	...
	public String insertContratante(ContratanteRequest contratanteRequest, UsuarioDto usuarioDto) {
		...
		q.agregarParametroValues(ID_PERSONA,
				0 < contratanteRequest.getIdPersona() ? contratanteRequest.getIdPersona().toString() : "idTabla1");
		q.agregarParametroValues(CVE_MATRICULA, SelectQueryUtil.setValor(contratanteRequest.getMatricula()));
		q.agregarParametroValues(ID_DOMICILIO,
				0 < contratanteRequest.getCp().getIdDomicilio() ? contratanteRequest.getCp().getIdDomicilio().toString()
						: "idTabla2");
	...
	private String insertPlanSfpa(PlanSFPARequest planSFPARequest, UsuarioDto usuarioDto, Map<String, String> map) {
		...
		q.agregarParametroValues(ConsultaConstantes.ID_ESTATUS_PLAN_SFPA, String.valueOf(8));
		q.agregarParametroValues(ConsultaConstantes.ID_USUARIO_ALTA, String.valueOf(usuarioDto.getIdUsuario()));
		q.agregarParametroValues(ConsultaConstantes.FEC_ALTA, ConsultaConstantes.CURRENT_DATE);
	...
	public String updatePlanSfpa(PlanSFPARequest planSFPARequest, UsuarioDto usuarioDto) {
		...
		q.agregarParametroValues(ConsultaConstantes.ID_ESTATUS_PLAN_SFPA,
				String.valueOf(planSFPARequest.getIdEstatusPlanSfpa()));
		q.agregarParametroValues(ConsultaConstantes.ID_USUARIO_MODIFICA, String.valueOf(usuarioDto.getIdUsuario()));
		q.agregarParametroValues(ConsultaConstantes.FEC_ACTUALIZACION, ConsultaConstantes.CURRENT_DATE);
	...
	public ArrayList<String> guardarPagoSFPA(PlanSFPARequest planSFPARequest, UsuarioDto usuarioDto) {
		...
		for (int i = 0; i < planSFPARequest.getNumPagoMensual(); i++) {
			...
			q.agregarParametroValues("FEC_PARCIALIDAD",ConsultaConstantes.COMILLA_SIMPLE.concat(fechafinal.format(DateTimeFormatter.ofPattern(YYYY_MM_DD))).concat(ConsultaConstantes.COMILLA_SIMPLE));
			q.agregarParametroValues(ConsultaConstantes.FEC_ALTA, ConsultaConstantes.CURRENT_DATE);
			q.agregarParametroValues(ConsultaConstantes.ID_USUARIO_ALTA, String.valueOf(usuarioDto.getIdUsuario()));
	...
	private Map<String, String> obtenerContrante(PlanSFPARequest planSFPARequest, ContratanteRequest contratanteRequest,
