...
public class ConsultaPlanSFPA implements Serializable {
	...
	public DatosRequest consultaPlanSFPA(DatosRequest request, ReporteRequest reporteRequest) {
		...
		queryUtil
				.select("DISTINCT SPLSFPA.ID_PLAN_SFPA AS ID_PLAN_SFPA",
						"SPLSFPA.NUM_FOLIO_PLAN_SFPA AS NUM_FOLIO_PLAN_SFPA",
						"CONCAT_WS(' ',SP.NOM_PERSONA,SP.NOM_PRIMER_APELLIDO,SP.NOM_SEGUNDO_APELLIDO ) AS TITULAR",
						"d.DES_DELEGACION AS ESTADO", "SP.REF_CORREO AS CORREO_ELECTRONICO",
						"SPE.REF_PAQUETE_NOMBRE AS PAQUETE", "TPM.DES_TIPO_PAGO_MENSUAL NUMERO_PAGO",
						"SEPLSFPA.DES_ESTATUS_PLAN_SFPA AS ESTATUS_PLAN_SFPA",
						"case  "
						+ "		when SEPLSFPA.ID_ESTATUS_PLAN_SFPA = 4 then 'PAGADO' "
						...
						+ "	END AS ESTATUS_PAGO_ANTICIPADO")
				.from("SVT_PLAN_SFPA SPLSFPA")
				.leftJoin("SVT_PAGO_SFPA SPSFPA", "SPLSFPA.ID_PLAN_SFPA = SPSFPA.ID_PLAN_SFPA")
				.innerJoin("SVC_CONTRATANTE SC", "SPLSFPA.ID_TITULAR = SC.ID_CONTRATANTE")
				.innerJoin("SVC_PERSONA SP", "SP.ID_PERSONA = SC.ID_PERSONA")
				.innerJoin("SVT_DOMICILIO SD", "SD.ID_DOMICILIO = SC.ID_DOMICILIO")
				.innerJoin("SVT_PAQUETE SPE", "SPE.ID_PAQUETE = SPLSFPA.ID_PAQUETE")
				...
				.innerJoin("SVC_DELEGACION d", "d.ID_DELEGACION = SV.ID_DELEGACION")
				.innerJoin("SVC_ESTATUS_PLAN_SFPA SEPLSFPA",
						"SEPLSFPA.ID_ESTATUS_PLAN_SFPA = SPLSFPA.ID_ESTATUS_PLAN_SFPA")
				.leftJoin("SVC_ESTATUS_PAGO_ANTICIPADO SEPA",
						"SEPA.ID_ESTATUS_PAGO_ANTICIPADO = SPSFPA.ID_ESTATUS_PAGO")
				.where("IFNULL(SPLSFPA.ID_PLAN_SFPA ,0) > 0");
		if (reporteRequest.getIdVelatorio() != null
				&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getIdVelatorio())) {
			queryUtil.and("SPLSFPA.ID_VELATORIO IN (" + reporteRequest.getIdVelatorio() + ")");
		}
		if (reporteRequest.getNumFolioPlanSfpa() != null
				&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getNumFolioPlanSfpa())) {
			queryUtil.and("SPLSFPA.NUM_FOLIO_PLAN_SFPA = :numFolioPlanSfpa").setParameter("numFolioPlanSfpa",
					reporteRequest.getNumFolioPlanSfpa());
		}
		if (reporteRequest.getRfc() != null && !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getRfc())) {
			queryUtil.and("SP.CVE_RFC = :rfc").setParameter("rfc", reporteRequest.getRfc());
		}
		if (reporteRequest.getCurp() != null && !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getCurp())) {
			queryUtil.and("SP.CVE_CURP = :curp").setParameter("curp", reporteRequest.getCurp());
		}
		if (reporteRequest.getNombreAfiliado() != null
				&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getNombreAfiliado())) {
			queryUtil.and("CONCAT_WS(' ',SP.NOM_PERSONA,SP.NOM_PRIMER_APELLIDO,SP.NOM_SEGUNDO_APELLIDO ) LIKE '%"
					+ reporteRequest.getNombreAfiliado() + "%'");
		}
		if (reporteRequest.getIdEstatusPlanSfpa() != null && 0 < reporteRequest.getIdEstatusPlanSfpa()) {
			queryUtil.and("SPLSFPA.ID_ESTATUS_PLAN_SFPA = :idEstatusPlanSfpa").setParameter("idEstatusPlanSfpa",
					reporteRequest.getIdEstatusPlanSfpa());
		}
		if ((reporteRequest.getFechaInicio() != null && reporteRequest.getFechaFin() != null)
				&& (!ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getFechaInicio())
						&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getFechaFin()))) {
			queryUtil.and("SPLSFPA.FEC_INGRESO  BETWEEN '" + reporteRequest.getFechaInicio() + "' AND '"
		...
		// String encoded =
		// DatatypeConverter.printBase64Binary(query.getBytes(StandardCharsets.UTF_8));
	...
	public String consultaPlanSFPA(ReporteRequest reporteRequest) {
		...
		if (reporteRequest.getNumFolioPlanSfpa() != null
				&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getNumFolioPlanSfpa())) {
			condicciones.append(" AND SPLSFPA.NUM_FOLIO_PLAN_SFPA = '").append(reporteRequest.getNumFolioPlanSfpa())
					.append(ConsultaConstantes.COMILLA_SIMPLE);
		}
		if (reporteRequest.getRfc() != null && !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getRfc())) {
			condicciones.append("AND SP.CVE_RFC = '").append(reporteRequest.getRfc())
					.append(ConsultaConstantes.COMILLA_SIMPLE);
		}
		if (reporteRequest.getCurp() != null && !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getCurp())) {
			condicciones.append("AND SP.CVE_CURP = '").append(reporteRequest.getCurp())
					.append(ConsultaConstantes.COMILLA_SIMPLE);
		}
		if (reporteRequest.getNombreAfiliado() != null
				&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getNombreAfiliado())) {
			condicciones.append(
					" AND CONCAT_WS(' ',SP.NOM_PERSONA,SP.NOM_PRIMER_APELLIDO,SP.NOM_SEGUNDO_APELLIDO ) LIKE '%")
					.append(reporteRequest.getNombreAfiliado()).append("%'");
		}
		if (reporteRequest.getIdEstatusPlanSfpa() != null && 0 < reporteRequest.getIdEstatusPlanSfpa()) {
		...
		if ((reporteRequest.getFechaInicio() != null && reporteRequest.getFechaFin() != null)
				&& (!ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getFechaInicio())
						&& !ConsultaConstantes.EMPTY_STRING.equals(reporteRequest.getFechaFin()))) {
			condicciones.append(" AND SPLSFPA.FEC_INGRESO  BETWEEN '").append(reporteRequest.getFechaInicio())
					.append("' AND '").append(reporteRequest.getFechaFin()).append(ConsultaConstantes.COMILLA_SIMPLE);
		}
